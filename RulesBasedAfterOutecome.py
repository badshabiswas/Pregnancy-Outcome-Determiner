...
for i, tweet in enumerate(X_test):
    ...
    rule_based_features = extract_rule_based_features(tweet)

    # Check if any rule is satisfied
    if any(rule_based_features):
        # Override the prediction based on the rules
        if rule_based_features[0] == 1:
            # Rule 1: Override if the rule-based feature is detected
            # Override prediction for this tweet to "Positive"
            y_pred[i] = "Positive"
        elif rule_based_features[1] == 1:
            # Rule 2: Override if another rule-based feature is detected
            # Override prediction for this tweet to "Negative"
            y_pred[i] = "Negative"
        elif rule_based_features[2] == 1:
            # Rule 3: Override if another rule-based feature is detected
            # Override prediction for this tweet to "Negative"
            y_pred[i] = "Negative"
